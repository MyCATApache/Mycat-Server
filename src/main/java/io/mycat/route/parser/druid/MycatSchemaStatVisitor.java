...
public class MycatSchemaStatVisitor extends MySqlSchemaStatVisitor {
	 ...
	 * 子查询集合
	 */
	private Queue<SQLSelect> subQuerys = new LinkedBlockingQueue<>();
	/**
	 * 是否有改写sql
	 */
...
//		Column reColumn = super.getColumn(expr);
//		if(reColumn == null && expr instanceof SQLBetweenExpr) {
...
//	}
	...
	public TableStat getTableStat(String tableName) {
		...
		return super.getTableStat(tableName);
	}

	@Override
	public TableStat getTableStat(SQLName tableName) {
		String strName = tableName.getSimpleName();
		if (strName != null && strName.indexOf("`") >= 0) {
			strName = strName.replaceAll("`", "");
		}
		long hashCode64 = FnvHash.hashCode64(strName);
...
//					}
	...
	public List<List<Condition>> splitConditions() {
		...
		List<List<Condition>> reList = mergedConditions();
		return reList;
	}
...
//				whereUnit.addSplitedExpr(expr.getRight());
	...
	public void setCurrentTable(SQLObject x, String table) {
		if(this.getTables().size()>0){
			Map.Entry<TableStat.Name, TableStat> entry = this.getTables().entrySet().iterator().next();
			x.putAttribute("_old_local_", entry.getKey().getName());
		}
		getTableStat(table);
	}
...
////		if (from == null || !from.getAlias().equals(((SQLIdentifierExpr) ((SQLExprTableSource) tableSource).getExpr()).getName())) {
