 ...
 */
public class MySQLGlobalConsistencyHelper implements SQLQueryResultListener<SQLQueryResult<Map<String, String>>> {
	private static final Logger LOGGER = LoggerFactory.getLogger(MySQLGlobalConsistencyHelper.class);
    private MySQLGlobalTableConsistencyChecker heartbeat;
    private volatile SQLJob sqlJob;
    private int RETRY_TIMES = 5;
    private AtomicInteger retryTime = new AtomicInteger();
    
    public MySQLGlobalConsistencyHelper(MySQLGlobalTableConsistencyChecker heartbeat, SQLJob sqlJob) {
    	this.heartbeat = heartbeat;
        this.sqlJob = sqlJob;
        this.retryTime.set(RETRY_TIMES);
    }
    
    public MySQLGlobalConsistencyHelper(MySQLGlobalTableConsistencyChecker heartbeat, 
    ...
    public MySQLGlobalTableConsistencyChecker getHeartbeat() {
        return heartbeat;
    }
