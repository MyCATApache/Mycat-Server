...
public class RecycleThread implements   Runnable {
	//回收bytebuffer数
	static AtomicInteger recycleCount = new AtomicInteger(0);
	...
	public void run() {
		...
		ByteBuffer theBuf = null;
		 for(;;){
			 ...
			 try {
				 // it won't be block until the queue has no bytebuf in it
				while((theBuf = DirectByteBufferPool.noBlockingQueue.take()) !=null){
 					 LOGGER.debug("recycleThread process : "+recycleCount.incrementAndGet()+" count,have : "+(DirectByteBufferPool.apply.get()-recycleCount.get() +" no deal"));
