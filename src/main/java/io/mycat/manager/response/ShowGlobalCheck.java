 ...
 * @author mycat
 */
public final class ShowGlobalCheck {
	...
	static {
		...
		fields[i] = PacketUtil.getField("hashCode", Fields.FIELD_TYPE_VAR_STRING);
		fields[i++].packetId = ++packetId;
	...
	private static RowDataPacket getRow(String tableName, String dataNode, String hashCode, String charset) {
		RowDataPacket row = new RowDataPacket(FIELD_COUNT);
		row.add(StringUtil.encode(tableName, charset));
		row.add(StringUtil.encode(dataNode, charset));
		row.add(StringUtil.encode(hashCode, charset));
		return row;
	}
